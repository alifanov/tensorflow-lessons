...
def plot_clusters(all_samples, centroids, n_samples_per_cluster):
    ...
    for i, centroids in enumerate(centroids):
        ...
        plt.plot(centroids[0], centroids[1], markersize=35, marker='x', color='k', mew=10)
        plt.plot(centroids[0], centroids[1], markersize=30, marker='x', color='m', mew=5)
    plt.show()
...
def assign_to_nearest(samples, centroids):
    ...
    distances = tf.reduce_sum(
        tf.square(
            tf.sub(expanded_vectors, expanded_centroids)
        ),
        2
    )
